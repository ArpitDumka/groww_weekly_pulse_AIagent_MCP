...
This milestone continues the same product you chose in **Milestone 1**. The goal is to turn raw mobile-store feedback into a **weekly pulse** your team can scan in minutes: what users care about, what they actually said, and what to do next.
...
Use **MCP (Model Context Protocol) servers** for Google Docs and Gmail—for example, creating or updating the pulse document and creating the draft message—rather than integrating Google APIs directly.

- **Do not** use bespoke OAuth client + REST client code as the primary integration path.
