"""Google Play Console review export parser."""
...
_RATING_ALIASES = ["star rating", "rating"]
_TITLE_ALIASES = ["review title", "title"]
_TEXT_ALIASES = ["review text", "review", "body"]
_DATE_ALIASES = [
    "review submit date and time",
    "review date",
    "submit date",
]
_VERSION_ALIASES = ["app version name", "app version"]
_LOCALE_ALIASES = ["device language", "language", "country code"]
...
def _to_review(raw: RawReviewRow) -> Review:
    return Review(
        id=make_review_id(ReviewSource.PLAY_STORE, raw.text, raw.review_date),
...
def parse_play_store_csv(path: Path) -> tuple[list[Review], ParseStats]:
    stats = ParseStats()
    reviews: list[Review] = []
    ...
    for index, row in enumerate(rows, start=2):
        try:
            ...
            if "missing required" in message or "empty" in message:
                ...
                logger.debug("Play Store row %s skipped (empty): %s", index, exc)
            else:
                stats.skipped_malformed += 1
                logger.debug("Play Store row %s skipped (malformed): %s", index, exc)
            continue
        ...
        try:
            ...
            stats.skipped_malformed += 1
            logger.debug("Play Store row %s invalid review: %s", index, exc)
